...
| 구분 | 경로 | 유형 | 주요 목적 |
| --- | --- | --- | --- |
| Arduino 장치 검색/제어 앱 | `BlazorApp_arduinoSearch_240824_01` | Blazor Server, .NET 10 | 로컬 네트워크 장치 검색, MQTT 연결, LED 제어, 센서 차트 표시 |
| 3D 대시보드 앱 | `BlazorApp3/BlazorApp3` | Blazor WebAssembly/PWA, .NET 10 | Blazor WASM 화면과 Unity WebGL 기반 3D 대시보드 로딩 |
...
- .NET 10 Blazor WebAssembly 프로젝트다.
...
현재 `dotnet restore`, `dotnet build --configuration Release --no-restore`, `dotnet test --configuration Release --no-build`를 실행했으며 다음 상태를 확인했다.

| 대상 | 결과 | 내용 |
| --- | --- | --- |
| `BlazorApp_arduinoSearch_240824_01.sln` | 성공 | Release 빌드 성공, 서버 테스트 2개 통과 |
| `BlazorApp3.sln` | 성공 | Release 빌드 성공 |

대상 프레임워크는 `net10.0`으로 업그레이드되어 기존 `net6.0` 지원 종료 경고는 해소됐다.
...
2. .NET 10 배포 확인
   - 개발 환경의 빌드/테스트는 통과했다.
   - 실제 배포 장비의 .NET 10 런타임 또는 호스팅 번들 설치 여부를 확인한다.
